public class Ksiazka {
    ...
    public Ksiazka wypozycz(String nazwa, String autor) throws Exception{
        ...
        bazaKsiazek.zaktualizuj(nazwa, autor, 1);
        return K;
    }

    public boolean rezerwuj(String nazwa, String autor) throws Exception{
        if ( !status(nazwa,autor) ){
            throw new Exception("Wybrana książka nie istnije lub jest niedostępna");
        }
        BazaKsiazek bazaKsiazek = new BazaKsiazek();
        bazaKsiazek.zaktualizuj(nazwa, autor, 2);
        return true;
    }

    public boolean zwroc(String nazwa, String autor) throws Exception{
        if ( !status(nazwa,autor) ){
            throw new Exception("Wybrana książka nie istnije lub jest niedostępna");
        }
        BazaKsiazek bazaKsiazek = new BazaKsiazek();
        bazaKsiazek.zaktualizuj(nazwa, autor, 0);
        return true;
    }

    public boolean przedluzWypozyczenie(String nazwa, String autor) throws Exception{
        if ( !status(nazwa,autor) ){
            throw new Exception("Wybrana książka nie istnije lub jest niedostępna");
        }
        BazaKsiazek bazaKsiazek = new BazaKsiazek();
